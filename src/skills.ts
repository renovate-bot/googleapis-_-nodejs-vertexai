...
export class Skills extends BaseModule {
  ...
  list = async (
    ...
    return new Pager<types.Skill>(
      'skills' as PagedItem,
      (x: types.ListSkillsRequestParameters) => this.listInternal(x),
   ...
   *
   * @param params Parameters for updating a Skill.
   * @return A Promise resolving to the updated Skill if waitForCompletion is true, or the SkillOperation otherwise.
   */
  async update(
    params: types.UpdateSkillRequestParameters,
  ): Promise<types.Skill | types.SkillOperation> {
    const config = params.config ? {...params.config} : {};
    ...
    if (displayName !== undefined) {
      updateMaskPaths.push('displayName');
    }
    ...
    if (localPath) {
      ...
      zippedFilesystemPayload = Buffer.concat(chunks).toString('base64');
      updateMaskPaths.push('zippedFilesystem');
    } else if (zippedFilesystemPayload != null) {
      if (
        typeof zippedFilesystemPayload === 'object' &&
        zippedFilesystemPayload instanceof Uint8Array
    ...
    config.updateMask = updateMaskPaths.join(',');
    config.zippedFilesystem = zippedFilesystemPayload;

    const requestParams: types.UpdateSkillRequestParameters = {
      ...params,
      config,
    };

    let operation = await this.updateInternal(requestParams);
    ...
    if (waitForCompletion) {
      ...
      if (operation.error) {
        throw new Error(
          `Failed to update Skill: ${JSON.stringify(operation.error)}`,
        );
      }
   ...
   * Deletes a Skill.
   *
   * @param params Parameters for deleting a Skill.
   * @return A Promise resolving to the DeleteSkillOperation if waitForCompletion is false, or null otherwise.
   */
  async delete(
    params: types.DeleteSkillRequestParameters,
  ): Promise<types.DeleteSkillOperation | null> {
    const config = params.config ? {...params.config} : {};
    const requestParams: types.DeleteSkillRequestParameters = {
      ...params,
      config,
    };

    let operation = await this.deleteInternal(requestParams);
    ...
    if (waitForCompletion) {
      while (!operation.done) {
        ...
        })) as unknown as types.DeleteSkillOperation;
      }

      if (operation.error) {
        throw new Error(
          `Failed to delete Skill: ${JSON.stringify(operation.error)}`,
        );
      }
  ...
  private async updateInternal(
    ...
    let queryParams: Record<string, string> = {};
    if (this.apiClient.isVertexAI()) {
      const body = converters.updateSkillRequestParametersToVertex(params);
      path = common.formatMap(
        '{name}',
        body['_url'] as Record<string, unknown>,
      );
  ...
  private async listInternal(
    params: types.ListSkillsRequestParameters,
  ): Promise<types.ListSkillsResponse> {
    let response: Promise<types.ListSkillsResponse>;

    let path: string = '';
    let queryParams: Record<string, string> = {};
    if (this.apiClient.isVertexAI()) {
      const body = converters.listSkillsRequestParametersToVertex(params);
      path = common.formatMap(
        'skills',
        body['_url'] as Record<string, unknown>,
      );
      ...
      response = this.apiClient
        ...
        .then((httpResponse) => {
          return httpResponse.json();
        }) as Promise<types.ListSkillsResponse>;

      return response.then((resp) => {
        const typedResp = new types.ListSkillsResponse();
  ...
  private async deleteInternal(
    params: types.DeleteSkillRequestParameters,
  ): Promise<types.DeleteSkillOperation> {
    let response: Promise<types.DeleteSkillOperation>;

    let path: string = '';
    let queryParams: Record<string, string> = {};
    if (this.apiClient.isVertexAI()) {
      const body = converters.deleteSkillRequestParametersToVertex(params);
      path = common.formatMap(
        '{name}',
        body['_url'] as Record<string, unknown>,
      );
      ...
      response = this.apiClient
        ...
        .then((httpResponse) => {
          return httpResponse.json();
        }) as Promise<types.DeleteSkillOperation>;

      return response.then((resp) => {
        return resp as types.DeleteSkillOperation;
      });
    } else {
      throw new Error(
        'This method is only supported by the Gemini Enterprise Agent Platform (previously known as Vertex AI).',
